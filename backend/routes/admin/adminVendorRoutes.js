...
router.get("/", protect, isAdmin, async (req, res) => {
  try {
    const vendors = await User.find({ role: "seller" }).sort({ createdAt: -1 });
    res.json(vendors);
  } catch (err) {
  ...
  try {
    ...
    await sendMail(
      ...
      `<div style="font-family:sans-serif;padding:20px">
        ...
        <p>Your seller account has been <b>APPROVED</b> by KashmirCraft Admin.</p>
        <p>You can now login and start selling.</p>
        <br/>
        <a href="https://ecommerce.softwalletinnovativetechnologies.cloud/auth">Login Now</a>
      </div>`
    );
...
router.put("/block/:id", protect, isAdmin, async (req, res) => {
  try {
    ...
    await sendMail(
      ...
      `<div style="font-family:sans-serif;padding:20px">
        <h2>Hello ${vendor.name}</h2>
        <p>Your seller account has been <b>BLOCKED</b> by KashmirCraft Admin.</p>
